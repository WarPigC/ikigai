...
// 1. ParticipantList.jsx
let pl = fs.readFileSync('frontend/src/components/student/ParticipantList.jsx', 'utf8');
// Change button
pl = pl.replace(
  /<button\s+onClick=\{.*?\}\s+className="w-10 h-10.*?flex items-center justify-center.*?>\s*<Plus size=\{20\} \/>\s*<\/button>/s,
  `<button onClick={() => setIsAddModalOpen(true)} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center justify-center font-medium shadow-sm transition-colors text-sm">
...
pl = pl.replace(
  /<th.*?Presenter Name<\/th>[\s\S]*?<th.*?Paper ID<\/th>[\s\S]*?<th.*?Title<\/th>[\s\S]*?<th.*?Email<\/th>/,
  `<th className="px-4 md:px-6 py-4 font-semibold text-gray-900 w-1/4">Team Name</th>
   <th className="px-4 md:px-6 py-4 font-semibold text-gray-900 w-1/4">Problem Statement</th>
   <th className="px-4 md:px-6 py-4 font-semibold text-gray-900 w-1/4 hidden md:table-cell">Team Leader</th>`
);

fs.writeFileSync('frontend/src/components/student/ParticipantList.jsx', pl);

// 2. ParticipantRow.jsx
let pr = fs.readFileSync('frontend/src/components/student/ParticipantRow.jsx', 'utf8');

pr = pr.replace(/<td className="px-4 md:px-6 py-4">[\s\S]*?<\/td>/,
...
<td className="px-4 md:px-6 py-4 text-gray-600 hidden md:table-cell">
  {participant.members?.find(m => m.isLeader)?.name || participant.members?.[0]?.name || '-'}
</td>`
);
// Remove the next 3 td elements (Paper ID, Title, Email) which we replaced.
// Since we used regex, it's safer to just rewrite the row component rendering.
pr = `import React, { useState } from "react";
...
fs.writeFileSync('frontend/src/components/student/ParticipantRow.jsx', pr);

console.log('Patched List and Row components');
